...
const currentUser = "employee2"; // يمكنك تغييرها في بيئتك الحقيقية
const currentUserRole = "admin"; // "admin" أو "employee"
const currentUserDepartment = "مطفي"; // القسم الخاص بالمستخدم (مثال)
...
function ChatBox({ deviceId, currentUser, currentUserDepartment }) {
  ...
  async function fetchMessages() {
    // جلب الرسائل الخاصة بهذا الجهاز فقط والتي تكون إما من admin أو من قسم مطفي
    const { data, error } = await supabase
      .from("device_notes")
      .select("*")
      .eq("device_id", deviceId)
      .in("sender", ["admin", "مطفي"]) // فقط الرسائل من هذين الطرفين
  ...
  async function sendMessage() {
    ...
    if (currentUser === "admin") {
      senderName = "admin";
    } else if (currentUserDepartment === "مطفي") {
      senderName = "مطفي";
    } else {
  ...
  return (
    <div className="border rounded p-3 bg-white shadow mt-2 max-h-60 overflow-y-auto">
      <div className="mb-2 font-bold text-right">
        محادثة الجهاز #{deviceId} (قسم: مطفي)
      </div>
          ...
          messages.map((msg) => (
            <div
              ...
              className={`p-2 rounded ${
                msg.sender === (currentUser === "admin" ? "admin" : "مطفي")
...
export default function DepartmentPage() {
  ...
  async function fetchDevices() {
    ...
    // جلب الأجهزة فقط لقسم مطفي سواء كان أدمن أو موظف، لا تعرض كل الأقسام هنا
    query = query.eq("department", "مطفي");
